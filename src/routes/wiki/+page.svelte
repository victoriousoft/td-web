<script lang="ts">
	import * as Card from "$lib/components/ui/card";
	import * as AlertDialog from "$lib/components/ui/alert-dialog";
	import Link from "$lib/components/link.svelte";
	import Button from "$lib/components/ui/button/button.svelte";
	import GlbView from "$lib/components/glb-view.svelte";

	let { data } = $props();

	function getRezistancePercentage(resistance: number) {
		return resistance * 25;
	}

	let towers: Record<string, string[]> = {
		magic: ["IBM", "Hackerman"],
		archers: ["Kittengun"],
		bomber: [],
		barrakcs: []
	};

	const towerLevels = [0, 1, 2];

	let currentTower = $state({
		name: "",
		description: "",
		price: 0,
		range: 0,
		damage: 0,
		cooldown: 0
	});
	let isTowerDialogOpen = $state(false);
</script>

<h1 class="m-5 text-center text-4xl font-bold">Wiki</h1>

<h2 class="m-5 text-2xl font-semibold">Enemies</h2>

<div class="m-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
	{#each data.enemies as enemy}
		{#if enemy.unlockLevel <= data.highestUnlockedLevel}
			<Card.Root>
				<Card.Header>
					<Card.Title>{enemy.name}</Card.Title>
				</Card.Header>
				<Card.Content>
					<div class="flex flex-col gap-2">
						<div class="mb-2 flex h-40 items-center justify-center">
							<img src={enemy.imageUrl} alt={enemy.name} class="max-h-40 max-w-full rounded-md object-contain" />
						</div>
						<p class="text-sm text-muted-foreground">{enemy.description}</p>
						<div class="mt-2 grid grid-cols-2 gap-2">
							<p><span class="font-semibold">Health:</span> {enemy.health}</p>
							<p><span class="font-semibold">Damage:</span> {enemy.damage}</p>
							<p><span class="font-semibold">Physical Resistance:</span> {getRezistancePercentage(enemy.physicalResistance)}%</p>
							<p><span class="font-semibold">Magical Resistance:</span> {getRezistancePercentage(enemy.magicalResistance)}%</p>
							<p><span class="font-semibold">Player Lives:</span> {"❤️".repeat(enemy.playerLives)}</p>
							<p><span class="font-semibold">Unlocked at level</span> {enemy.unlockLevel}</p>
						</div>
					</div>
				</Card.Content>
			</Card.Root>
		{:else}
			<Card.Root class="opacity-75">
				<Card.Header>
					<Card.Title>??? Unknown Enemy</Card.Title>
				</Card.Header>
				<Card.Content>
					<div class="flex flex-col gap-2">
						<div class="mb-2 flex h-40 items-center justify-center">
							<div class="flex h-full w-full items-center justify-center rounded-md bg-muted">
								<span class="text-4xl">?</span>
							</div>
						</div>
						<p class="text-sm text-muted-foreground">This enemy is still unknown to you.</p>
						<div class="mt-2">
							<p class="font-semibold text-red-500">Unlocked after completing level {enemy.unlockLevel + 1}</p>
						</div>
					</div>
				</Card.Content>
			</Card.Root>
		{/if}
	{/each}
</div>

<h2 class="m-5 text-2xl font-semibold">Towers</h2>

<div class="m-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
	{#each Object.keys(towers) as towerName}
		<Card.Root>
			<Card.Header>
				<Card.Title>{towerName.charAt(0).toUpperCase() + towerName.slice(1)}</Card.Title>
			</Card.Header>
			<Card.Content>
				<div class="flex flex-col gap-2">
					{#each towerLevels as towerLevel}
						<div class="mt-2 grid grid-cols-2 gap-2">
							<Button
								onclick={() => {
									let newTower = data.towersMap.get(`${towerName} ${towerLevel + 1}`);
									if (!newTower) {
										console.error("Tower not found");
										return;
									}
									currentTower = {
										name: newTower.name,
										description: newTower.description,
										price: newTower.price,
										range: newTower.range,
										damage: newTower.damage,
										cooldown: newTower.cooldown
									};
									isTowerDialogOpen = true;
								}}
							>
								Level {towerLevel + 1}
							</Button>
						</div>
					{/each}
					{#each towers[towerName] as evoTowerName}
						<div class="mt-2 grid grid-cols-2 gap-2">
							<Button
								onclick={() => {
									let newTower = data.towersMap.get(evoTowerName.toLowerCase());
									if (!newTower) {
										console.error("Tower not found");
										return;
									}
									currentTower = {
										name: newTower.name,
										description: newTower.description,
										price: newTower.price,
										range: newTower.range,
										damage: newTower.damage,
										cooldown: newTower.cooldown
									};
									isTowerDialogOpen = true;
								}}
							>
								{evoTowerName}
							</Button>
						</div>
					{/each}
				</div>
			</Card.Content>
		</Card.Root>
	{/each}
</div>

<AlertDialog.Root bind:open={isTowerDialogOpen}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title>Tower overview</AlertDialog.Title>
			<AlertDialog.Description>
				<GlbView modelUrl="http://minio.kristn.co.uk:9000/ratartd-models/IBM.glb"></GlbView>

				<p class="text-sm text-muted-foreground">{currentTower.description}</p>
				<div class="mt-2 grid grid-cols-2 gap-2">
					<p><span class="font-semibold">Price:</span> {currentTower.price}</p>
					<p><span class="font-semibold">Damage:</span> {currentTower.damage}</p>
					<p><span class="font-semibold">Range:</span> {currentTower.range}</p>
					<p><span class="font-semibold">Cooldown:</span> {currentTower.cooldown}s</p>
				</div>
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
			<AlertDialog.Cancel>Cancel</AlertDialog.Cancel>
			<AlertDialog.Action>Continue</AlertDialog.Action>
		</AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>
